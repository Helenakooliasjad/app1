...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER necessity: {necessity};
    TASK: Write a product description that is tailored into this customer's Age group and necessity. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and necessity;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account necessity {necessity} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...";OUTPUT TEXT in Estonian; 
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "necessity", "content"],
    template=template,
)
...
with col1:
    st.markdown("Otstarve: tootetutvustustekstide personaliseerimine igale kliendile või kliendigruppidele; väljundtekst on kohandatud kliendi a) vanuserühmaga ja b) vajadusega; sisendtekstiks on neutraalses vormis tootekirjeldus. \
    \n\n Kasutusjuhend: 1) valmista ette tootekirjeldus (sisendtekst). 2) määra tarbijasegemendid lähtuvalt vanuserühma ja vajaduse kombinatsioonidest. 3) sisesta ükshaaval tarbijasegmentide lõikes eeltoodud info äpi kasutajaliideses, saada ära. \
...
def get_necessity():
    input_text = st.text_input(label="Customers necessity", key="necessity_input")
    return input_text

hobby_input = get_necessity()
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, necessity=hobby_input, content=content_input)
